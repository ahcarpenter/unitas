...
// circle back to flatten out tables
 ...
 */
export const createTable = mysqlTableCreator((name) => `unitas_${name}`);
...
export const authenticators = createTable(
  "authenticator",
  (d) => ({
    credentialID: d.varchar("credentialID", { length: 255 }).notNull().unique(),
    userId: d
    ...
    credentialPublicKey: d
      .varchar("credentialPublicKey", {
        length: 255,
      })
      .notNull(),
    counter: d.int("counter").notNull(),
    credentialDeviceType: d
      .varchar("credentialDeviceType", {
        length: 255,
      })
      .notNull(),
    credentialBackedUp: d.boolean("credentialBackedUp").notNull(),
    transports: d.varchar("transports", { length: 255 }),
  }),
  (authenticator) => ({
